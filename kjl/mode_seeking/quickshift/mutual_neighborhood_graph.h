...
//    printf("\nmaximum of %d and %d is = %d",a,b,c); // instead of cout, using print
    ...
    for (int c = 0; c < n_chosen_clusters; ++c) {
//      std::cout << n_chosen_clusters << ","<< modal_sets[0].size() << "," << modal_sets[0].size()-1 << ","<< static_cast<int>(modal_sets[0].size()-1) <<endl;
...
//      https://stackoverflow.com/questions/22184403/how-to-cast-the-size-t-to-double-or-int-c
        for (int i = 0; i < static_cast<int>(modal_sets[c].size())-1; ++i) {
//            std::cout << i <<","<< static_cast<int>(modal_sets[c].size())-1 << endl;
