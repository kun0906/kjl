	...
	def fit(self, X, thres=0.95, n_comp_thres=20, **kwargs):
		...
		if self.seek_name.upper() == 'quickshift'.upper():
			"""Initialize GMM
				...
				# Note: (no sklearn.neighbors.kde for higher scikit-learn version)
				quickshift++ requries an old sikit-learn (scikit-learn~=0.23.2 works) and old python3.7.9
...
def meanshift_seek_modes(X, bandwidth=None, thres_n=100):
	...
	print(
		f'--all clusters ({all_n_clusters}) when (bandwidth:{bandwidth}, {clustering.bandwidth}). However, only {n_clusters} '
...
#     print(f"number of neighbors in k-NN: {k}")
#     # Declare a Quickshift++ models with tuning hyperparameters.
#     models = QuickshiftPP(k=k, beta=beta)
#
#     # Note the try catch cannot capture the models.fit() error because it is cython. How to capture the exception?
#     try:
#         models.fit(X)
