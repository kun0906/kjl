...
class Pcap(PCAP):
	...
	def show(self, src_ptks, name='src', out_file='res.pdf'):
		...
		for i, (ip, pkts) in enumerate(tops):
			...
			if i == len(tops) - 1:
				is_xlabel = True
			self._show(ax[i, 0], x, y, xlabel='', ylabel=ylabel, label=f'{name}:{ip}', title='', is_xlabel=is_xlabel,
			           is_ylabel=False)
		...
		for i, (ip, pkts) in enumerate(tops):
			...
			y = [n_pkts for start_time, n_pkts in split_pkts_info]
			ylabel = f'Num. of pkts'
			self._show(ax, x, y, xlabel='', ylabel=ylabel, label=f'{name}:{ip}', title='', is_xlabel=True,
			           is_ylabel=True)
...
def main():
	in_file = 'offline/data/deeplens_open_shut_fridge_batch_8.pcap_filtered.pcap'
