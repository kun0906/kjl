...
class KJL:
    ...
    def fit(self, X_train, y_train=None, X_train_raw=None, y_train_raw=None):
        ...
        if independent_row_col:
            ...
            indRow = resample(range(N), n_samples=self.n, random_state=self.random_state, stratify=y_train,
                              replace=False)
            indCol = resample(range(N), n_samples=self.m, random_state=self.random_state, stratify=y_train,
                              replace=False)
        else:
            # Random select max(n,m) rows
            # indices = np.random.randint(N, size=max(n, m))
            indices = resample(range(N), n_samples=max(self.n, self.m), random_state=self.random_state, stratify=y_train,
                               replace=False)
        ...
        np.random.seed(self.random_state)
        self.random_matrix = np.random.multivariate_normal([0] * self.d, np.diag([1] * self.d), self.m)  # mxd
