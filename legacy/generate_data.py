...
#             expand_out_dir = os.path.join(out_dir, os.path.dirname(normal_pcap_file.split('applications/')[-1]))
...
#                         normal_pcap_file.split('applications/')[-1].replace(_name, '-'.join(datasets).replace('/', '-'))))
...
#         data = mimic_data(datasets, random_state=100)
...
def generate_data(data_name, data_type='two_datasets', out_file='.dat', overwrite=False, random_state=42):
    ...
    if data_name == 'mimic_GMM':
        ...
        if data_name == 'UNB1_UNB2':
            subdatasets = ('DS10_UNB_IDS/DS11-srcIP_192.168.10.5', 'DS10_UNB_IDS/DS12-srcIP_192.168.10.8')  # each_data has normal and abnormal
        ...
        if data_name == 'UChi_FRIG1': # Fridge: idle and open_shut
            subdatasets1 = ('DS60_UChi_IoT/iotlab_open_shut_fridge_192.168.143.43/idle', )
            subdatasets2 = ('DS60_UChi_IoT/iotlab_open_shut_fridge_192.168.143.43/idle',
                            'DS60_UChi_IoT/iotlab_open_shut_fridge_192.168.143.43/open_shut')
            subdatasets = (subdatasets1, subdatasets2)

            normal_files = [] # store normal flows
            abnormal_files = [] # store abnormal flows
            # normal_0
            flows = get_flows('DS60_UChi_IoT/iotlab_open_shut_fridge_192.168.143.43/idle',)
            out_file = in_dir + '/'
            dump_data(flows, out_file='normal_0')
            normal_files.append(out_file)
            # abnormal_0
            flows = get_flows('DS60_UChi_IoT/iotlab_open_shut_fridge_192.168.143.43/open_shut')
            out_file = in_dir + '/'
            dump_data(flows, out_file='normal_0')
            ...
            subdatasets = ('DS60_UChi_IoT/iotlab_open_shut_fridge_192.168.143.43/idle', 'DS60_UChi_IoT/iotlab_open_shut_fridge_192.168.143.43/browser')
...
if __name__ == '__main__':
    ...
    _generate_mimic_data(data_type='two_datasets', random_state=42, out_file='./data/misc.dat')
