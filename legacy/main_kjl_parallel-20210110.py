...
def _generate_datasets(overwrite=False, direction= 'src_dst'):
    ...
    in_dir = f'offline/data/{direction}/{feat}'
...
#         ocsvm_results = single_main(data=datasets, params=(case, experiment))
...
#                                           n_repeats=n_repeats, random_state=random_state,
#                                           verbose=verbose, overwrite=overwrite)
#         _new_results = single_main(data=datasets, params=(case, experiment))
...
#     # save results first
#     out_dir = f'offline/out/iat_size-gs_{is_gs}-{covariance_type}-std_{is_std}_center_{is_std_mean}'
...
def _main_default_single( n_repeats = 5, is_gs=False, is_std=False, covariance_type='diag', n_jobs=16, n_comp_default=1,
                         q_default=0.3, n_kjl_default=500, d_kjl_default=10):
    results = {}
    ...
    print(f'\n\nfinal results: {results}')
    # save results first
    out_dir = f'offline/out/{direction}/iat_size-gs_{is_gs}-{covariance_type}-std_{is_std}_center_{is_std_mean}/' \
...
def _main_default(n_repeats = 5, is_gs=False, is_std=False, covariance_type='diag', n_jobs=16):
    is_try = False
    if is_try:
...
@execute_time
def _main_best( n_repeats = 5, is_gs=False, is_std=False, covariance_type='diag', n_jobs=16):
    results = {}
...
def main():
    ...
    covariance_types =['diag', 'full'] #['diag', 'full']
    stds = [False]
    n_repeats = 5
    ...
    for i, (is_gs, is_std, covariance_type) in enumerate(itertools.product(gses, stds, covariance_types)):
        ...
        if is_gs:
            _main_best(n_repeats = n_repeats,is_gs=is_gs, is_std=is_std, covariance_type=covariance_type)
        else:
            _main_default(n_repeats = n_repeats,is_gs=is_gs, is_std=is_std, covariance_type=covariance_type)
