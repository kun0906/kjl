...
from online.generate_data import PCAP2FEATURES, mimic_data
...
from kjl.utils.data import load_data, extract_data, dump_data, save_result, batch, data_info
from kjl.utils.tool import execute_time, time_func
from collections import Counter
from online.report import plot_result
...
class BASE_MODEL():
    ...
    def get_best_model(self, X_train, y_train, X_test, y_test):
        ...
        for n_components, kjl_d, kjl_n, kjl_q in list(itertools.product(params['n_components'],
            ...
            # Fit a models on the train set
            self._init_train(X_train, y_train)  # fit self.models with self.params on train set
        ...
        ##########################################################################################
        print('\n\n******best models')
        # Step 3: get the best models with best_params
        ...
        # Fit a models on the init set
        self._init_train(X_train, y_train)  # fit self.models
        # Evaluate the models on the test set
    ...
    def _init_train(self, X_train, y_train=None):
        ...
        # self.models.X_train_proj = X_train  # X_train:  after standardization and projection
        self.model.sum_resp = np.sum(np.exp(self.model.log_resp), axis=0)
        self.model.y_score = y_score
        self.init_X_train_proj = X_train  # self.models.X_train_proj = X_train
...
class ONLINE_GMM_MAIN(BASE_MODEL, ONLINE_GMM):
    ...
    def _online_train(self, X_batch, y_batch=None):
        ...
        if self.verbose >= 8:
            print(f"online models prediction takes {model_predict_time} seconds.")
        ...
        ##########################################################################################
        # Step 2: online train a new models on the batch data
        ...
        # print('self.models.weights_', self.models.weights_)
        # print('self.models.means_', self.models.means_)
        # print('self.models.covariances_', self.models.covariances_)
        # new_model.n_samples = self.models.n_samples
    ...
    def _online_test(self, X_test, y_test):
        ...
        print(
            ...
            f', proj_test_time: {proj_test_time}, '
            f'model_test_time: {model_test_time}, models.paramters: {self.model.get_params()}, auc: {self.auc}')
...
def split_train_arrival_test(normal_X, normal_y, abnormal_X, abnormal_y, one_data_flg=False, random_state=42):
    ...
    # datasets 2
    # arrival set: data1/data2 = 1:9
    if one_data_flg:  # use the same datasets
...
#     obs_dir = '../../IoT_feature_sets_comparison_20190822/applications/'
...
def main(random_state, n_jobs=-1, n_repeats=1, online=True):
    ...
    datasets = [  # 'DEMO_IDS/DS-srcIP_192.168.10.5',
        ...
        # ('DS10_UNB_IDS/DS13-srcIP_192.168.10.8', 'DS10_UNB_IDS/DS14-srcIP_192.168.10.9'),   # misc
    ...
    for i, subdatasets in enumerate(datasets):
        Xy_file = get_path(subdatasets, '../examples/online/out/data/data_reprst/pcaps', name='Xy-normal-abnormal.dat')
