...
def show_3train_sizes_n_clusters(in_files, out_file='', title='auc', n_repeats=5, show_name='n_clusters'):
    ...
    for i, ((model_name, k_qs), in_file) in enumerate(in_files.items()):
        ...
        df = pd.DataFrame(sub_dataset, columns=['datasets', 'model_name', 'train_size','n_clusters'])
        # g = sns.barplot(y="diff", x='datasets', data=df, hue='repres', ax=axes[t, ind % fig_cols], palette=new_colors)
...
if __name__ == '__main__':
    ...
    if only_show_ocsvm:
        in_dir = 'offline/out_train_sizes'  # neon
        # in_dir = 'offline/out_train_sizes/keep_small_clusters'  # neon
