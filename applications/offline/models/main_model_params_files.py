""" Main function: load built models from disk and evaluate them on test data
    ...
    1. Instructions for executing the main file ('main_load_model.py')
        ...
        3) Execute the main file
            mkdir offline/out
            PYTHONPATH=../:./ python3.7 offline/main_load_model.py > offline/out/main_load_model.txt 2>&1 &
    ...
    Here, we want to evaluate five models on three datasets.
        ...
        3) test memory
            ...
            PYTHONPATH=../:./ ~/.local/bin/scalene offline/main_load_model-dev.py
    ...
    2. Python libraries
        ...
        1) odet==0.0.1         # extract features
            i) cd "kjl/kjl/datasets/odet-master"
            ii) pip3.7 install .

        2) QuickshiftPP==1.0        # seek modes
            i) cd "kjl/kjl/models/quickshift/"
...
def save_model_params(model, out_file):
    ...
    if 'OCSVM' in model.params['model_name']:
        # if models.params['model_name'] == 'OCSVM(rbf)':
...
def _main(in_dir, data_name, model_name, feat_set='iat_size', is_gs=True, is_header=False):
    ...
    for i in range(n_repeats):
        try:
            ...
            model_file = pth.join(in_dir, f'repeat_{i}.models')
            ...
            # test_time = []
            # for _ in range(num):
            #     auc_, test_time_ = models.test(X_test, y_test)
            #     auc.append(auc_)
            #     test_time.append(test_time_)
            # auc = np.mean(auc)
            # test_time = np.mean(test_time)
...
def clean(in_dir, dataset_name="CTU1", model_name="OCSVM(rbf)", feat_set='iat_size', is_gs=True, is_header=True, start_time=None):
    ...
    for i in range(n_repeats):
        try:
            ...
            for tmp_file in [f'repeat_{i}.models', f'Test_set-repeat_{i + 1}.dat', 'results.dat', 'res.dat.csv',
                             'res.dat']:
...
if __name__ == '__main__':
    in_dir_src = 'offline/out/kjl_serial_ind_32_threads-cProfile_perf_counter-20times-4'
    in_dir_src = 'offline/out/kjl_serial_ind_32_threads-cProfile_perf_counter-20times-5'
