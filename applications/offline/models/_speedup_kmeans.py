"""Main entrance
    ...
    PYTHONPATH=../:./ python3.7 offline/main_kjl_parallel.py > out/main_kjl_parallel.txt 2>&1 &
"""
...
from kjl.models.kjl import getGaussianGram, kernelJLInitialize
from kjl.models.nystrom import nystromInitialize
from kjl.models.ocsvm import OCSVM
from kjl.models.seek_mode import quickshift_seek_modes, meanshift_seek_modes
...
class KMeans_MAIN(BASE):
    ...
    def train_test_intf(self, X_train, y_train, X_test, y_test):
        ...
        print(f'models.get_params(): {model.get_params()}')
        # train models
        try:
        ...
        # if self.models.covariance_type == 'full':
        #     # space_size = (d ** 2 + d) * n_comps + n * (d + D)
        #     space_size = (d ** 2 + d) * self.models.n_components + n * (d + D)
        # elif self.models.covariance_type == 'diag':
        #     # space_size = (2* d) * n_comps + n * (d + D)
        #     space_size = (2 * d) * self.models.n_components + n * (d + D)
        # else:
        #     msg = self.models.covariance_type
...
class OCSVM_MAIN(BASE):
    ...
    def train_test_intf(self, X_train, y_train, X_test, y_test):
        ...
        # set models default parameters
        model.set_params(**model_params)
        print(f'models.get_params()L {model.get_params()}')
        # train models
        try:
            self.model, self.model_train_time = self._train(model, X_train)
...
#             if params['mode'] =='best':
#                 models.train_test_intf(X_train, y_train, X_val, y_val)
#             elif params['mode'] == 'default':
#                 models.train_test_intf(X_train, y_train, X_test, y_test)
#
...
#         print('train_test')
#
#         info = {'train_time': models.train_time, 'test_time': models.test_time, 'auc': models.auc, 'apc': '',
#                 'params': models.params, 'space_size': models.space_size,
#                 'X_train_shape': X_train.shape, 'X_test_shape': X_test.shape}
