""" Main function
    1. Instructions for executing the main file
        1) Change the current directory to "applications/"
            cd applications/
        2) Check the python3 veriosn
            python3 -V
        3) Execute the main file
            PYTHONPATH=../:./ python3.7 offline/main_kjl.py > offline/out/main_kjl.txt 2>&1 &
    ...
    2) Execute the main file
        PYTHONPATH=../:./ python3.7 mprof run -multiprocess offline/main_kjl.py > offline/out/main_kjl.txt 2>&1 &
        # " PYTHONPATH=../:./ python3.7 ~/.local/lib/python3.7/site-packages/mprof.py  run —multiprocess
        # offline/main_kjl.py > offline/out/main_kjl.txt 2>&1 & "
...
from offline._merge import _dat2csv, merge_res
from offline._speedup_kjl import _get_single_result
...
DATASETS = [
    ...
    ################################################################################################################
    # SMTV datasets. All smtv datasets are on NOEN server: /opt/smart-tv/roku-data-20190927-182117
...
def _get_model_cfg(model_cfg, n_repeats=5, q=0.3, kjl_n=100, kjl_d=5, n_comp=1,
    ...
    """ Get all params needed for an experiment except for datasets params
...
class SingleEXP:
    def __init__(self, in_dir=None, n_repeats=5, q=0.3, kjl_n=100, kjl_d=5, n_comp=1,
                 random_state=42, verbose=10, overwrite=False, n_jobs=10):
        """Generate the result by one algorithm on one datasets.
    ...
    def get_model_cfg(self, model_cfg):
        """get all models cfg based on model_cfg
    ...
    def execute(self, model_cfg=None, data_cfg=None):
        """build and evaluate a models on a data
...
def single_main(data_cfg, model_cfg, out_dir=''):
    """ Get the result by one models on one datasets
    ...
    # Store the result obtained by the models on the datasets
    res = []

    try:
        ################################################################################################################
        # 1. Initialize the parameters
        exp = SingleEXP(in_dir=f'offline/data', n_repeats=5,
        ...
        ################################################################################################################
        # 3. Get the models's parameters
        model_cfg = exp.get_model_cfg(model_cfg)
        model_cfg['out_dir'] = out_dir

        ################################################################################################################
        # 4. Run the models and get the result
...
def parse_cmd_args():
    ...
    parser.add_argument("-d", "--datasets", help="datasets", default="CTU1")
...
def main1(DATASETS = [], MODELS = [],
        directions=[('direction', 'src_dst'), ],
          ...
          gses=[('is_gs', True), ('is_gs', False)],
          before_projs=[('before_proj', False), ],
          ds=[('kjl_d', 5), ], out_dir='offline/out',
...
def copy_merge_res(in_dir, out_dir):
    ...
    print(current_path) # # 'kjl/applications/
    zip_file = f'{current_path}/offline/neon_train_out.zip'
    os.chdir(in_dir)
...
def main(dataset = 'CTU1', model='OCSVM(rbf)', start_time = None):
    # out_dir = 'offline/out/kjl_parallel_30'
    out_dir = 'offline/out/kjl_serial_ind_32_threads'
    out_dir = 'offline/out/kjl_serial_ind_1_thread'
    out_dir = 'offline/out/kjl_serial_ind_32_threads-cProfile_perf_counter'
    # out_dir = 'offline/out/kjl_serial_ind_1_thread-cProfile_perf_counter'
    # out_dir = 'offline/out/kjl_serial_ind_32_threads-cProfile_perf_counter-libsvm'
    out_dir = 'offline/out/kjl_serial_ind_32_threads-cProfile_perf_counter-2'
    out_dir = 'offline/out/kjl_serial_ind_32_threads-cProfile_perf_counter-20times-4'
    out_dir = 'offline/out/kjl_serial_ind_32_threads-cProfile_perf_counter-20times-5'

    # # # out_dir = 'offline/out/kjl-with_init_params'    # modify __speedup_kjl.py
    # DATASETS = [datasets]
    # MODELS = [models]
    # try:
    #     ###########################################################################################################
