            ...
            try:
                if sheet_name == 'OCSVM':
                    ...
                    line = "Dataset   & UNB      & CTU  & MAWI         & MACCDC       & SFRIG  & AECHO   &DWSHR " \
                           + ' \\\\' + '\n\midrule'
                    line += '\nAUC &' + " & ".join([v.replace('+/-', '$\pm$') for v in outs[0][1][1:]])  \
                           + ' \\\\' + '\n\midrule' \
                        '\n\\Cell{Server Train \\\\Time (ms)} & ' + " & ".join(
                        [v.replace('+/-', '$\pm$').split('(')[0] for i_, v in enumerate(outs[3][2][1:])]) \
                            + ' \\\\' + '\n\midrule'\
                        '\n\Cell{Test Time (ms):\\\\\hspace{-0.1cm}RSPI} & ' + " & ".join([v.replace('+/-', '$\pm$') for v in outs[0][3][1:]]) \
                           + ' \\\\' + '\n\cmidrule{2-8}' \
                      '\n\\Cell{\hspace{0.1cm}NANO} & ' + " & ".join([v.replace('+/-', '$\pm$') for v in outs[1][3][1:]]) \
                           + ' \\\\' + '\n\cmidrule{2-8}' \
                      '\n\Cell{\hspace{0.cm}Server} & ' + " & ".join([v.replace('+/-', '$\pm$') for v in outs[2][3][1:]]) \
                           + ' \\\\'+ '\n\midrule'\
                    ...
                    while i < len(outs[0]):
                        ...
                        #         + ' \\\\' + '\n\cmidrule{2-6}' \
                        #     '\n& \Cell{Train Time Speedup\\\\\\hspace{-1.0cm}Server} & ' + " & ".join([v.replace('+/-', '$\pm$') for i_, v in enumerate(outs[3][i + 1]) if i_ not in [0, 1, 2, 5, 6]]) \
                        #         + ' \\\\' +  '\n\cmidrule{2-6}' \
                        ...
                        line += f'\n{DATASETS[j]}' +'   & AUC retained &' + " & ".join([v.replace('+/-', '$\pm$') for i_, v in enumerate(outs[0][i]) if i_ not in [0, 1,2, 5,6]])\
                                + ' \\\\' + '\n\cmidrule{2-6}' \
                            '\n& {Train Time Speedup\\\\\\hspace{-1.0cm}Server} & ' + " & ".join([v.replace('+/-', '$\pm$') for i_, v in enumerate(outs[3][i + 1]) if i_ not in [0, 1, 2, 5, 6]]) \
                                + ' \\\\' +  '\n\cmidrule{2-6}' \
                            '\n& {Test offline:\\\\\\hspace{-1.0cm}RSPI} & ' + " & ".join([v.replace('+/-', '$\pm$') for i_, v in enumerate(outs[0][i+2]) if i_ not in [0, 1, 2, 5, 6]]) \
                                + ' \\\\'  + '\n\cmidrule{3-6}' \
                            '\n& {\\hspace{0.4cm} NANO} & ' + " & ".join([v.replace('+/-', '$\pm$') for i_, v in  enumerate(outs[1][i+2]) if i_ not in [0, 1, 2, 5, 6] ]) \
                                + ' \\\\'  + '\n\cmidrule{3-6}' \
                            '\n& {\\hspace{0.4cm} Server}  & ' + " & ".join([v.replace('+/-', '$\pm$') for i_, v in enumerate(outs[2][i+2]) if i_ not in [0, 1, 2, 5, 6]]) \
                                + ' \\\\'   + '\n\cmidrule{2-6}' \
                            '\n& Space Saving & ' + " & ".join([v.replace('+/-', '$\pm$') for i_, v in enumerate(outs[0][i+3]) if i_ not in [0, 1, 2, 5, 6]]) \
                                + ' \\\\'   + '\n\midrule'
