"""Main entrance
    ...
    python3 -V
    PYTHONPATH=../:./ python3.8 offline/main_kjl.py > offline/out/main_kjl.txt 2>&1 &
    ...
    PYTHONPATH=../:./ python3.8 mprof run -multiprocess offline/main_kjl.py > offline/out/main_kjl.txt 2>&1 &
    PYTHONPATH=../:./ python3.7 ~/.local/lib/python3.7/site-packages/mprof.py  run —multiprocess offline/main_kjl.py > offline/out/main_kjl.txt 2>&1 &
...
from offline._speedup_kjl import single_main
from offline.generate_data import generate_data_speed_up
from offline._merge import _dat2csv, merge_res, merge_ds_res
...
def _main(data_cfg, model_cfg, out_dir =''):
    ...
    try:
        exp = SingleEXP(in_dir= f'offline/data', n_repeats=5,
                        q=0.3, n_kjl=100, d_kjl=model_cfg['d_kjl'], n_comp=1,
...
def main(directions=[('direction', 'src_dst'), ],
         ...
         gses=[('is_gs', True), ('is_gs', False)],
         before_projs=[('before_proj', False), ],
         ds=[('d_kjl', 5), ], out_dir = 'offline/out',
         is_parallel=True):
    ...
    models_cfg = list(itertools.product(models, gses, before_projs, ds))
    ...
    if is_parallel:
        ...
        parallel = Parallel(n_jobs=6, verbose=30)
        with parallel:
...
if __name__ == '__main__':
    ...
    #     #      )
    #     main(feats=[('feat', 'stats'), ('feat', 'iat_size')],
    #          # headers=[('is_header', False)],
    #          # gses=[('is_gs', False)],
    #          before_projs=[('before_proj', False), ],
    #          ds=[('d_kjl', 5), ],
    ...
    res_type = 'ds'  # merge tuning ds results
    if res_type=='ds':
        in_dir = 'offline/out_ds' # neon
        # in_dir = 'offline/calumet_out_ds-20210201'    # calumet
        merge_ds_res(in_dir=in_dir, datasets= ['SFRIG1_2020', 'AECHO1_2020'],
                  directions=[('direction', 'src_dst'), ],
                  feats=[('feat', 'stats'), ],
                  ...
                  # gses=[('is_gs', False)],
                  before_projs=[('before_proj', False), ],
                  ds=[('d_kjl', v) for v in [5] + list(range(10, 100+1, 10))]
                  )
    else:
        in_dir = 'offline/out'  # neon
        # in_dir = 'offline/calumet_out-20210204' # calumet
        merge_res(in_dir=in_dir, datasets= DATASETS,
                  directions=[('direction', 'src_dst'), ],
                  feats=[('feat', 'iat_size')],
