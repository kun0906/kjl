...
class Environment():
    ...
    def step(self, action, y=0, wait_time=0, reward_method='add'):
        ...
        if reward_method == 'add':
            ...
            if action == y:
                ...
                if action == 0 and y == 1:  # FN
                    ...
                    reward = -1 * (2**wait_time)
                done = False
        elif reward_method == 'misc':   # try to use as many as pkts
            if action == y:
                if action == 0:  # normal
...
class RL():
    ...
    def __init__(self, in_dim=2, random_state=42):
        ...
        self.actions = ['normal', 'abnormal', 'unsure(wait)'] # ['pass', 'alarm', 'unsure'] #  'none'
    ...
    def select_action(self, state, mode='train'):
        ...
        if mode == 'test':
            ...
            with torch.no_grad():
                ...
                # second column on max result is index of where max element was
                # found, so we pick action with the larger expected reward.
                # y_pred=self.policy_net(state).max(1)[1].view(1, 1)
                y_proba = self.policy_net(state)

            action = np.random.choice([0, 1, 2], 1, p=y_proba.data.numpy().flatten())
