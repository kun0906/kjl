...
# class RewardNet():
#     def __init__(self, in_dim=2, out_dim=2):
#         self.models = NET(in_dim=in_dim, out_dim=out_dim)
#         self.epoches = 10
#
#         # create your optimizer
#         self.optimizer = optim.SGD(self.models.parameters(), lr=0.01)
#         self.criterion = nn.MSELoss()
#
#     def fit(self, X, y):
#         self.models.train()  # Train = True
#         for epoch in range(self.epoches):
#             # for X_, y_ in zip(X, y ):
#             outs = self.models(X)
#             outs = outs.max().flatten()     # choose the max one
