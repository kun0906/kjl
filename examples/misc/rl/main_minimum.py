...
class Environment():
    ...
    def step(self, action, X, theta, y_previous=1, wait_time=0, reward_method='add'):

        # action = [0, 1, 2]
        theta = update_f(X, theta, action)
        y = f(X, theta)
        y_true = f(X, [2, 1])
        loss = np.mean([(v1-v2)**2 for (v1, v2) in zip(y, y_true)]).item()
        print(f'step_loss: {loss}')
        if loss < y_previous : # '+'
            reward = 10
            done = False
        else:
        ...
        return None, reward, done, {'y': loss, 'theta': theta}
...
class RL():
    ...
    def train(self, X, y):
        ...
        train_loss_info = []
        y_info = []
        for i_episode in range(100):  # each flow is an episodes
            ...
            X_batch = [np.random.randint(-10, 10) ** 2 for v in range(10)]  # f = 2*x**2 + 1, w = 2, b = 1
            theta = [10, 5]  # w=10, b= 5
            y = f(X_batch, theta)
            y_true =f(X_batch, [2, 1])
            y_previous = np.mean([(v1 - v2) ** 2 for (v1, v2) in zip(y, y_true)]).item()
            state = torch.Tensor(theta).view(1, -1)   # f(initial) = 100
            pkt_0_time = 0
            i_pkt = 0
            ...
            while True and i_pkt < 30:
                ...
                _, reward, done, meta_info = self.env.step(action.item(), X_batch, theta, y_previous=y_previous, wait_time=wait_time)
                tot_reward = reward   # reward is the instant reward after taking an action, total_reward += gamma**i_pkt * reard
                rewards_.append((reward, i_pkt))
                y_previous = meta_info['y']
                X_batch = [np.random.randint(-10, 10) ** 2 for v in range(10)]  # f = 2*x**2 + 1, w = 2, b = 1

                if not done:
                    if i_pkt + 1 < 100:
                        # next_state = (torch.Tensor(X_[i_pkt]), torch.Tensor([y_]))
                        next_state = meta_info['theta']
                        theta = next_state
                ...
                state = next_state
                print(f'i: {i_episode}, i_pkt: {i_pkt}, theta: {theta}')
            ...
            print(f'i: {i_episode}, theta: {theta}, loss: {loss}')
            # Update the target network, copying all weights and biases in DQN
            if i_episode % self.TARGET_UPDATE == 0:
                self.target_net.load_state_dict(self.policy_net.state_dict())
            y_info.append((y_, meta_info['y']))
            rewards_info.append((tot_reward, rewards_))
            pkts_info.append((i_pkt + 1,  1))
