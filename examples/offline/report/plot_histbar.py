...
def plot_bar_difference_seaborn(data=[], show_detectors=['OCSVM', 'AE'],
	...
	for j, detector_name in enumerate(show_detectors):
		...
		df = pd.DataFrame(sub_dataset, columns=['datasets', 'repres', 'diff'])
		# g = sns.barplot(y="diff", x='datasets', data=df, hue='repres', ax=axes[t, ind % fig_cols], palette=new_colors)
		...
		g = sns.barplot(y="diff", x='datasets', hue='repres', data=df, palette=new_colors, ci=None,
		                capsize=.2, ax=axes[t, j % c])
		ys = []
		xs = []
		width = 0
	...
	for j, diff_name in enumerate(metrics):
		...
		for data_name, ds in res.items():
			for model_name, vs in ds.items():
				...
				if diff_name == 'Speedup AUC':
					sub_dataset.append([data_name, f"{model_name}", mean_, std_])
				elif diff_name in ['Speedup training', 'Speedup testing', 'Saving space']:
					sub_dataset.append([data_name, f"{model_name}", mean_, std_])
		...
		print(f'{diff_name}: {sub_dataset}')
		df = pd.DataFrame(sub_dataset, columns=['datasets', 'model_name', 'offline', 'std'])
		# colors = [ 'green', 'orange', 'c', 'm',  'b', 'r','tab:brown', 'tab:green'][:2]
		g = sns.barplot(y="offline", x='datasets', hue='model_name', data=df, ci=None,
		                capsize=.2, ax=axes[t, j % c])
		ys = []
		xs = []
		width = 0
		sub_fig_width = 0
		yerrs = []
		for name_, groups_vs in df.groupby('model_name'):
			# print(name_, groups_vs.values)
			yerrs.extend(list(groups_vs['std'].values))
		...
		axes[t, j % c].errorbar(x=xs + width / 2, y=ys,
		                        yerr=yerrs, fmt='none', c='r', capsize=3)
...
def show_oc_kjl_qs_vs_nystrom_qs2(res, out_file='output_data',
	...
	t = 0
	metrics = ['AUC Retained', 'Train Time Speedup', 'Space Saving', 'Test Time Speedup']
	for j, diff_name in enumerate(metrics):
		sub_dataset = []
		for data_name, ds in res.items():
			for device_name in ['RSPI', 'NANO', 'NEON']:
				for model_name, vs in ds.items():
					if type(vs) == dict:
						...
						vs = vs.split('+/-')
						mean_, std_ = float(vs[0]), float(vs[1])
						if diff_name == 'AUC Retained' and device_name == 'NEON':
							sub_dataset.append([data_name, f"{model_name}", mean_, std_])
						elif diff_name in ['Train Time Speedup', 'Space Saving'] and device_name == 'NEON':
							sub_dataset.append([data_name, f"{model_name}", mean_, std_])
						elif diff_name in ['Test Time Speedup']:
							if device_name == 'NEON':
								sub_dataset.append([data_name, f"{model_name}(Server)", mean_, std_])
							else:
								sub_dataset.append([data_name, f"{model_name}({device_name})", mean_, std_])
		...
		print(f'{diff_name}: {sub_dataset}')
		df = pd.DataFrame(sub_dataset, columns=['datasets', 'model_name', 'offline', 'std'])
		# colors = [ 'green', 'orange', 'c', 'm',  'b', 'r','tab:brown', 'tab:green']
		colors = sns.color_palette()
		g = sns.barplot(y="offline", x='datasets', hue='model_name', data=df, ci=None, palette=colors,
		                capsize=.2, ax=axes[t, j % c])
		ys = []
		xs = []
		width = 0
		sub_fig_width = 0
		yerrs = []
		for name_, groups_vs in df.groupby('model_name', sort=False):
			print(name_, groups_vs.values)
			yerrs.extend(list(groups_vs['std'].values))
...
def show_oc_kjl_qs_full_vs_oc_kjl_qs_diag(res, out_file='output_data',
	...
	t = 0
	metrics = ['AUC Retained', 'Train Time Speedup', 'Space Reduction', 'Test Time Speedup']
	for j, diff_name in enumerate(metrics):
		sub_dataset = []
		yerrs = []  # std/sqrt(n_repeats)
		res_ = res['full']
				...
				for covarince_type in ['full', 'diag']:
					...
					for model_name, vs in ds.items():
						...
						if type(vs) == dict:
							...
							mean_, std_ = float(vs[0]), float(vs[1])
							if diff_name == 'AUC Retained' and device_name == 'NEON':
								sub_dataset.append([data_name, f"{model_name} / OCSVM", mean_, std_])

							elif diff_name in ['Train Time Speedup', 'Space Reduction'] and device_name == 'NEON':
								sub_dataset.append([data_name, f"{model_name}", mean_, std_])
...
def oc_kjl_qs_vs_oc_nystrom_qs(feat_set='iat_size', is_header=False, is_gs=False, covariance_type='full',
	...
	for i, device_name in enumerate(['RSPI', 'NANO', 'NEON']):
		if device_name == 'NEON':
			...
			train_out_dir = 'offline/paper_data/neon_train_out/out/'
			...
			test_out_dir = f'offline/paper_data/neon_out/out/models_res/'
			...
			sub_dir = f'src_dst/{feat_set}-header_{is_header}/before_proj_False-gs_{is_gs}' \
			          f'/std_False_center_False-d_5-{covariance_type}'
			# test_out: includes AUCs, testing time and spaces
			test_out_dir = f'offline/paper_data/{device_name.lower()}_out/out/models_res'
