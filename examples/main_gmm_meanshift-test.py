...
class SINGLE_CASE:
    ...
    def run(self, params):
        ...
        for is_online in [True, False]:
            ...
            _info = {}
            for i in range(params_copy.n_repeats):
                ...
                params_copy.X_arrival, params_copy.y_arrival, params_copy.X_test, params_copy.y_test = split_train_arrival_test(X, y, params_copy)
                if i == 0 and is_online:
                    X_tmp = np.concatenate([params_copy.X_init_train, params_copy.X_init_test, params_copy.X_arrival, params_copy.X_test],
                                       axis=0)
                    y_tmp = np.concatenate([params_copy.y_init_train, params_copy.y_init_test, params_copy.y_arrival, params_copy.y_test],
                                       axis=0)
    ...
    def display(self, info, out_file, key=(), is_show=False):
        ...
        if kjl:
            # {covariance_type};
            title = f'n_cp={n_components_init}, {covariance_type}; gs={gs}; std={std}_c={with_means_std}; KJL={kjl}, n={n_kjl}, d={d_kjl}, q={q_kjl}; ms={meanshift}'
            if fixed_kjl:
                title = f'Fixed KJL on {dataset_name}; n_repeats={n_repeats}\n{title}'
            elif fixed_U_size:
                # (replace {n_point} cols and rows of U)
                title = f'Fixed U size on {dataset_name}; n_repeats={n_repeats}\n{title}'
            else:  # increased_U
                title = f'Increased U size on {dataset_name}; n_repeats={n_repeats}\n{title}'
        else:
            #  {covariance_type};
            title = f'n_cp={n_components_init}, {covariance_type}; gs={gs}; std={std}_ctr={with_means_std}; KJL={kjl}; ms={meanshift}'
            title = f'{dataset_name};n_repeats={n_repeats}\n({title})'
...
class ONLINE_GMM_MAIN(BASE_MODEL, ONLINE_GMM):
    ...
    def batch_train_test(self, X_arrival, y_arrival, X_test, y_test):
        ...
        for i, (X_batch, Y_batch) in enumerate(batch(X_arrival, y_arrival, step=self.params.batch_size)):
            ...
            # online test model
            X_test1, preprocessing_time = time_func(self._preprocessing, X_test)
            self.online_test(X_test1, preprocessing_time, y_test)
    ...
    def online_train(self, X_batch, y_batch=None):
        ...
        if self.verbose >= 1:
            ...
            mprint(f'X_acculumated_train_proj: {X_acculumated_train_proj.shape}, y_acculumated_train: {Counter(self.y_acculumated_train)}')
        best_n_iter_ = 0
    ...
    def online_test(self, X_test, preprocessing_time, y_test):
...
class BATCH_GMM_MAIN(BASE_MODEL):
    ...
    def batch_train_test(self, X_arrival, y_arrival, X_test,
        ...
        for i, (X_batch, Y_batch) in enumerate(batch(X_arrival, y_arrival, step=self.params.batch_size)):
            ...
            # batch test model
            X_test1, preprocessing_time = time_func(self._preprocessing, X_test)
            self.batch_test(X_test1, preprocessing_time, y_test)
            self.info['test_times'].append(self.test_time)
            self.info['aucs'].append(self.auc)
    ...
    def batch_train(self, X_batch, y_batch=None):
        ...
        if self.params.fixed_kjl:  # (fixed KJL, U, and STD)
            ...
            self.model = GMM(n_components=self.model.n_components, covariance_type=self.params.covariance_type,
                             random_state=self.params.random_state)
            self._init_train(copy.deepcopy(self.X_acculumated_train), self.y_acculumated_train, is_batch_train=True)  # self.train_time
            self.train_time = {'train_time': train_time + self.train_time['train_time'],
                               ...
                               'seeking_time': self.train_time['seeking_time'],
    ...
    def batch_test(self, X_test, preprocessing_time, y_test):
        # if self.verbose >= DEBUG: data_info(np.asarray(X_test), name='X_test')
...
def main(n_init_train=500):
    ...
    out_file = f'out/{in_dir}/init={n_init_train}-gs={gs}-std={std}_center={with_means_std}-kjl={kjl}-d_kjl={d_kjl}-n_kjl={n_kjl}-ms={ms}-fixed_kjl={fixed_kjl}.dat'
    # print(out_file)
