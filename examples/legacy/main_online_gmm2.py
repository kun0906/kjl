...
class OFFLINE_GMM_MAIN(BASE):
    ...
    def train_test_intf(self, X_train, y_train, X_test, y_test):
        ...
        Parameters
        ----------
        X_train: array with shape(n_samples, n_feats)

        y_train: array with shape(n_samples, n_feats)

        X_test: array with shape(n_samples, n_feats)

        y_test: array with shape(n_samples, n_feats)
        ...
        ##########################################################################################
        # training phase:
        # 1. split train set into two subsets: initial set (init_set) and new arrival set (arvl_set) with ratio 3:7.
        ...
        self._train_test_intf(X_train, y_train, X_test, y_test)

        init_train_time = self.train_time
        init_test_time = self.test_time
        print(f'init_train_time: {init_train_time}, init_test_time: {init_test_time}')

        # self._train_test_intf(X_arrival, y_arrival, X_test, y_test)
        train_time_arr = []
        test_time_arr = []
        auc_arr = []
        for x, y in zip(X_arrival, y_arrival):
            X_train = np.concatenate([X_train, x.reshape(1, -1)], axis=0)
            y_train = np.concatenate([y_train, y.reshape(1, )], axis=0)
            self._train_test_intf(X_train, y_train, X_test, y_test)
            train_time_arr.append(self.train_time)
            test_time_arr.append(self.test_time)
            auc_arr.append(self.auc)

        print(f'train_time (init_set + arvl_set): {self.train_time}, test_time: {self.test_time}')

        self.train_time_arr = train_time_arr
        self.test_time_arr = test_time_arr
        self.auc_arr = auc_arr

        return self

    def _train_test_intf(self, X_train, y_train, X_test, y_test):
        self.train_time = 0
        self.test_time = 0
...
class ONLINE_GMM_MAIN(BASE, ONLINE_GMM):
    ...
    def train_test_intf(self, X_train, y_train, X_test, y_test):
        ...
        self._offline_train(X_train, y_train)
        self._offline_test(X_test, y_test)
    ...
    def _offline_train(self, X_train, y_train):
        # should do standardization before using pairwise_distances
    ...
    def _offline_test(self, X_test, y_test):
...
def main(random_state, n_jobs=-1, n_repeats=1):
    ...
    for data_name in datasets:
        ...
        # 2. try to make case more smaller and specific
        gs = True
        cases = [  # OCSVM-gs:True
        ...
        for case in cases:
            ...
            if 'gs' not in case.keys():
                ...
                if case['detector_name'] == 'OCSVM':
                    ...
                    case['offline_gmm'] = False
                    # case['offline_gmm'] = False if case['online_gmm'] else case['offline_gmm']
