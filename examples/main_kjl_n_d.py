...
@execute_time
def main_tuning_kjl(random_state, n_jobs=-1, n_repeats=1):
    datasets = [
        ...
        for case in cases:
            ...
            case_str = '-'.join([f'{k}_{v}' for k, v in case.items() if
                                 k in ['detector_name', 'covariance_type', 'gs', 'kjl', 'nystrom', 'quickshift',
                                       'meanshift']])
            case['kjl_ns'] = [10, 50, 100, 150, 200, 250, 300, 350, 400, 450, 500]
            case['kjl_ds'] = [2, 5, 8, 10, 20, 40, 60, 80, 100, 200]

            if case_str == 'detector_name_GMM-covariance_type_full-gs_True-kjl_True-quickshift_False-meanshift_False-nystrom_False':
                if in_expand_dir == f'data/data_kjl/DS10_UNB_IDS/DS12-srcIP_192.168.10.8/iat_size/header:False':
                    case['n_components'] = [5]
                    case['kjl_qs'] = [0.4]

                elif in_expand_dir == f'data/data_kjl/DS40_CTU_IoT/DS42-srcIP_192.168.1.196/iat_size/header:False':
                    case['n_components'] = [5]
                    case['kjl_qs'] = [0.2]

                elif in_expand_dir == f'data/data_kjl/DS50_MAWI_WIDE/DS52-srcIP_203.78.7.165/iat_size/header:False':
                    case['n_components'] = [45]
                    case['kjl_qs'] = [0.9]

                elif in_expand_dir == f'data/data_kjl/ISTS/2015/iat_size/header:False':
                    case['n_components'] = [45]
                    case['kjl_qs'] = [0.2]

                elif in_expand_dir == f'data/data_kjl/MACCDC/2012/iat_size/header:False':
                    case['n_components'] = [35]
                    case['kjl_qs'] = [0.4]

                elif in_expand_dir == f'data/data_kjl/DS60_UChi_IoT/DS62-srcIP_192.168.143.42/iat_size/header:False':
                    case['n_components'] = [40]
                    case['kjl_qs'] = [0.7]
                else:
                    print(f'not find {in_expand_dir}')
                    continue

            else:
                print(f'case_str: {case_str}')
                traceback.print_exc()
                continue
            try:
                # get each result
                ...
                dump_data(_middle_results, out_file + '-middle_results-kjl.dat')
    ...
    # save results first
    out_file = f'{out_dir}/all_results-kjl.csv'
    print(f'\n\n***{out_file}***')
...
if __name__ == '__main__':
    main_tuning_kjl(random_state=RANDOM_STATE, n_jobs=-1, n_repeats=1)
